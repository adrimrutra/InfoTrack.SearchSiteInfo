...
To update the database use this command from the Web project folder (replace `InfoTrack.SearchSiteInfo` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../InfoTrack.SearchSiteInfo.Infrastructure/InfoTrack.SearchSiteInfo.Infrastructure.csproj -s InfoTrack.SearchSiteInfo.Web.csproj
```
